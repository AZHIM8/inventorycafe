...
class BarangController extends Controller
{
    ...
    public function destroy(Barang $barang)
    {
        ...
        $gambarPath = public_path(Storage::url($barang->gambar));
        ...
        if (file_exists($gambarPath)) {
            ...
            // Jika file tidak ditemukan, Anda bisa mengembalikan pesan atau log
            \Log::warning('Gambar tidak ditemukan: ' . $gambarPath);
        }

        // Menghapus data barang
        try {
            $barang->delete(); // Menggunakan metode delete() pada model
        } catch (\Exception $e) {
            return response()->json([
                'success' => false,
                'message' => 'Terjadi kesalahan saat menghapus data barang: ' . $e->getMessage()
            ]);
        }
